...
def _find_available_port(start: int = 9004, end: int = 9010) -> int:
    """9004-9010の範囲で使用可能なポートを探す"""
    import socket
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("FINTECH_PORT", "0"))
    if port == 0:
        port = _find_available_port()
        if port != 9004:
            print(f"*** ポート9004が使用中のため、ポート{port}で起動します ***")
            print(f"*** フロント .env に REACT_APP_FINTECH_PLATFORM_URL=http://localhost:{port} を設定 ***\n")
