...
@router.post(
    "/pipelines", response_model=CICDPipelineModel, status_code=status.HTTP_201_CREATED
)
...
async def create_cicd_pipeline(
    ...
    """CI/CDパイプラインを作成"""
    stages = (
        [PipelineStage(s) for s in pipeline_data.stages]
...
async def trigger_pipeline(
    ...
    try:
        run = cicd_pipeline.trigger_pipeline(
            pipeline_id=pipeline_id, commit_hash=trigger_data.commit_hash
        )
        return run
...
@router.post(
    "/applications", response_model=Application, status_code=status.HTTP_201_CREATED
)
@require_permission("manage_infrastructure")
async def register_application(
    ...
    """アプリケーションを登録"""
    environments = (
        [Environment(e) for e in app_data.environments]
