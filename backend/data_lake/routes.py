...
async def list_buckets(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    """バケット一覧を取得"""
    try:
...
@router.post(
    "/buckets", response_model=BucketResponse, status_code=status.HTTP_201_CREATED
)
async def create_bucket(
    ...
    try:
        ...
        if not created:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Bucket already exists"
            )
...
async def delete_bucket(
    bucket_name: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
    """バケットを削除"""
    try:
...
async def upload_file(
    ...
    try:
        ...
        minio_client.upload_file(
            bucket_name, object_name, file_data, content_type=file.content_type
        )
...
@router.post(
    "/catalog", response_model=DataCatalogEntry, status_code=status.HTTP_201_CREATED
)
...
async def get_catalog_entry(
    catalog_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
    """カタログエントリを取得"""
...
async def update_catalog_entry(
    ...
    """カタログエントリを更新"""
    entry = catalog.update(catalog_id, **catalog_data.dict(exclude_unset=True))
    if not entry:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Catalog entry not found"
        )
...
async def get_policy_for_bucket(
    ...
    if not policy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Policy not found for bucket"
        )
    return policy
