...
class RAGSystem:
    ...
    def _ensure_default_docs(self):
        ...
        default_docs = [
            ("uep_docs", "UEP v5.0 は次世代エンタープライズ統合プラットフォームです。MLOps、生成AI、監視、セキュリティ、クラウドインフラを統合しています。", {"source": "README"}),
            ("uep_docs", "RAG（Retrieval-Augmented Generation）は、外部知識を検索してLLMの入力に加え、根拠のある回答を生成する技術です。ハルシネーションを軽減します。", {"source": "AI"}),
            ("uep_docs", "CoT（Chain-of-Thought）推論は、段階的に思考を進めることで複雑な問題を解く手法です。", {"source": "AI"}),
            ("uep_docs", "Contract Testing は、APIの契約（リクエスト・レスポンス形式）を検証するテスト手法です。", {"source": "品質"}),
            ("uep_docs", "Chaos Engineering は、障害を意図的に注入してシステムのレジリエンスを検証する手法です。", {"source": "品質"}),
        ]
