...
try:
    ...
    import logging
    logger = logging.getLogger(__name__)
    logger.warning(f"GraphQL module not available (pip install strawberry-graphql[fastapi]): {e}")
...
async def _chaos_ok_handler():
    return {"status": "ok", "instance_id": _chaos_instance_id, "message": "Chaos/GraphQL"}
async def _chaos_status_handler():
    return {
        "enabled": True,
        "endpoints": {"delay": "/api/v1/chaos/delay", "error": "/api/v1/chaos/error", "mixed": "/api/v1/chaos/mixed"},
        "description": "障害シミュレーション用エンドポイント（開発・検証環境向け）",
    }
app.add_api_route("/chaos-ok", _chaos_ok_handler, methods=["GET"], include_in_schema=False)
app.add_api_route("/api/v1/chaos/status", _chaos_status_handler, methods=["GET"], tags=["Chaos Engineering"])
...
if GRAPHQL_AVAILABLE and graphql_router:
    ...
    async def graphql_fallback():
        return {"message": "GraphQL not available. Install: pip install strawberry-graphql[fastapi]"}
...
async def add_request_id(request: Request, call_next):
    ...
    try:
        ...
        public_paths = ["/", "/health", "/metrics", "/docs", "/redoc", "/openapi.json", "/api/v1/health", "/graphql", "/chaos-ok"]
        is_public_path = request.url.path in public_paths or request.url.path.startswith("/docs") or request.url.path.startswith("/static") or request.url.path.startswith("/graphql") or request.url.path.startswith("/api/v1/chaos")
...
async def root():
    ...
    return {
        ...
        "levels": ["Level 1: Enterprise", "Level 2: Platform", "Level 3: Ecosystem", "Level 4: Industry Leader", "Level 5: Global (design)"],
        "docs": "/docs",
        "health": "/health",
        "api": "/api/v1",
        "graphql": "/graphql",
...
async def list_services(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    """登録されているサービス一覧（認証必須）"""
    return {
        "services": [
            {
